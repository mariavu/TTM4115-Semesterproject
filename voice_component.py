...
filename="output.wav" #file to send
...
class Voice_component:
    ...
    def create_gui(self):
        ...
        self.app.startLabelFrame('start recording and send')
        def on_button_pressed_start(title):
            name = extract_timer_name(title)
            self.r.record(10,filename)
            fo=open(filename,"rb")
            chunk=fo.read()
        ...
        self.app.addButton('start recording and send', on_button_pressed_start)
